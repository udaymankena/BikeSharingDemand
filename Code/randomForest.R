...
set.seed(500)
fit <- randomForest(as.factor(count) ~ hour + month + year + weekday + weather + atemp +
                      workingday + holiday + windspeed + humidity + season, data=train, ntree = 700, importance=TRUE)

prediction <- predict(fit, test)

# Create submission dataframe and output to file
submit <- data.frame(datetime = test$datetime, count = as.numeric(prediction))
write.csv(submit, file = "Data/r_random_forest.csv", row.names = FALSE)
